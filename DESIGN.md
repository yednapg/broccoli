...
The search field should stay optically aligned when the user starts typing. Files and Clipboard show their scope as a small token ahead of the magnifier; the query, placeholder, caret, and inline text follow the magnifier through the shared search geometry. The token's title is centered by layout, not by a fixed-height label. Result growth should happen in complete rows. Selection, focus, and inactive states should remain easy to read in Light and Dark appearances. Position the live launcher by dragging its chrome. AppKit hands that gesture to Window Server so the panel stays with the pointer; Broccoli stores the origin when the drag ends. Do not move the panel with a mouse-tracking setFrame loop. Opening from the keyboard restores the stored origin. Height changes keep that top edge and grow downward.
...
System Settings results use native pane artwork from installed Settings extensions. The catalog may resolve that artwork in the background after discovery, at utility priority with a small worker cap so warmup cannot spike CPU or RAM. Visible rows start immediately at a higher priority and are not queued behind the rest of the catalog. They replace empty slots as each source finishes. Do not paint an SF Symbol, custom tile, or border while that artwork is loading, and do not flatten the arrived pane art into a white SF Symbol when the row is selected. Pane results carry the System Settings application icon as a small badge at the artwork's bottom-right corner, so they stay distinguishable from applications while Liquid Glass hides subtitles. The badge appears only once the pane artwork has arrived and keeps its colors when selected; the System Settings application result itself has no badge.

Application and Settings artwork is materialized once per drawing context and then reused. Resolved bitmaps persist between launches, so a cold launch or a relaunch after quitting does not wait on the system icon service again. A visible row never starts a second materialization of artwork a warmup is already going to produce, and presenting the launcher does not re-sample artwork whose appearance, contrast, size and scale are unchanged. When the system answers with its generic placeholder instead of the real icon, that answer is shown but never persisted, and it is retried rather than left in place.
...
Result rows, selection, and text commit immediately and nonanimated. While the panel is visible, height changes run as one short interruptible animation of the window frame and its content in a single transaction, keeping the top edge fixed (growth is downward only). First presentation (hotkey) and dismissal remain nonanimated. Reduce Motion collapses every transition to the instant commit. Input and result delivery never wait for motion; a new query interrupts and retargets the transition instantly. Commit result rows, surface geometry, and the window size as one update before drawing when motion is disabled or interrupted. The launcher must not inherit animations from unrelated view updates. All listed result kinds use the theme's standard row height and result-container insets. Liquid Glass search and result rows share the same height. Center status symbols and text within that shared row; do not introduce status-specific heights or adjust them independently. When a main-launcher query matches nothing, Search's Web Search setting decides the presentation: Off shows “No results” beside the query and the launcher stays a single search band; Google or DuckDuckGo shows one selectable search row whose mark sits on an application-icon tile that is white in Light and dark in Dark. A number still being typed does not offer that row. A calculation that cannot be solved does. Return on the inline “No results” message does nothing. The query leaves the Mac only when the user chooses the web-search row, and it never reaches learned usage or recents. An empty file search shows only the field and its placeholder; do not add a row that repeats the prompt. Do not open the result area for a status message that cannot be selected, including an unfinished calculation and a file search with no matches. A collapse keeps the rows that are already showing and lets the shrinking window clip them, including the surplus when a shorter list replaces a longer one; clearing those rows first leaves an empty expanded surface. While the window grows or fully collapses, its rounded bottom edge is the only edge that reveals or clips the rows: the result viewport keeps its height instead of following the window, and the header rule fades in or out as that edge crosses the top result inset. A viewport pinned above the bottom inset cuts the rows along a second, straight edge, leaves a strip of empty glass to close after a collapse, and makes AppKit enlarge the compact bar to the insets' minimum before a growth begins, so the rule appears on an empty strip that then opens beneath it. The appearance-toggle action describes the opposite of the current system appearance, independently of a launcher appearance override; update its wording even when search work began before the system changed.

### Liquid Glass

Liquid Glass is the more expressive launcher style, but the material should never become the point of the design. The compact search capsule and expanded launcher use the same corner radius, derived from half the shared search-row height, with continuous corners. One glass clip inside the surface rounds the material and the content, and it is the only layer that antialiases the outline. The Dark rim is drawn once above that clip with the same corner geometry. A rim stroked inside the outline and clipped by it again loses part of its outer pixels along the arcs but not along the straight edges, so it thins abruptly where each arc begins. Expansion changes the surface's height, not its corner curvature or material configuration. Selection uses the macOS accent color in both appearances, with semantic selected text and the existing inset geometry.

In Light, one behind-window `NSVisualEffectView` with the `hudWindow` material is the whole surface, and the launcher content lives inside it so labels keep its vibrancy. Round it with the surface's glass clip, an ancestor layer, not by layer-backing the effect view or adding a mask image that antialiases the same edge a second time. Light query text, placeholder, magnifier, and cancel control share the magnifier's device black; the header rule uses that ink at 25% opacity.

In Dark, the surface is SwiftUI's `regularMaterial` darkened by 36% neutral black. Every AppKit material blends the wallpaper toward gray, which reads as a dusty veil over saturated wallpaper; Spotlight's dark surface instead stays neutral dark gray over white and deep in the wallpaper's own hue over color. Nothing in Dark adds a hue. The ink is opaque neutral gray composited with plus-lighter, so it brightens the surface beneath it: the placeholder and magnifier add 50%, the typed query 88%, the header rule 12%, and a one-point rim along the surface edge adds 12.5%, or 22% with Increase Contrast. Selection stays the solid accent color. Content moves above the SwiftUI backdrop when the appearance becomes Dark and back into the HUD for Light; move it before hiding the HUD, because hiding an ancestor of the first responder ends the edit and discards marked text. Keep the HUD in the window and set it inactive while Dark is showing. An active HUD under the SwiftUI material samples as well, so the surface draws the wallpaper and that second background together, and both resample on every height step. The SwiftUI backdrop fills its bounds and lives inside the glass clip, which gives it the capsule outline; do not give it its own shape clip or layer mask. SwiftUI adapts the Dark material to Reduce Transparency and Increase Contrast, as AppKit does for the HUD.

Configure both backdrops once; appearance is inherited from its panel or preview and selects which one samples. Draw `xmark.circle.fill` as a template so the X stays knocked out; a palette fill turns that control into a solid disc. Catalog colors over HUD wallpaper read brown. Do not stack a second active backdrop, tint, opacity, or blur layer on either material.
...
The borderless window must fit the surface exactly: do not inset it inside a larger transparent window, because the native window shadow then has a second boundary. Everything the surface draws stays inside its shared corner geometry, updating bounds without animation so rectangular corner pixels cannot extend beyond it; do not clip the window content a second time. Enable the floating panel's native window shadow and invalidate its shape after the capsule expands or collapses. Apart from the Dark plus-lighter rim, do not overlay a custom stroke on the material's edge. Use the same surface and selection in production and Settings previews. Do not substitute a separate Light material or a custom opaque Liquid Glass surface. Detached previews cannot reproduce the native window shadow or desktop blur.
...
- Search owns result sources, adaptive ranking, and the web-search fallback for queries that match nothing. The fallback is Off, Google, or DuckDuckGo. A number still being typed does not offer it; a calculation that cannot be solved does.
...
General presents one Global Shortcut row; clicking the displayed shortcut starts recording, and registration status is that row’s subtitle. Shortcut troubleshooting is an error-only path. The Calculator Supported card keeps capabilities and the example in one padded child. Capability symbols share the leading edge of one column, so every title in a column starts on the same line. Grouped Settings surfaces clip to the 12-point rounded rect. The Clipboard History off banner is notice-only; enabling still uses the existing toggle and consent sheet. Power actions describe a second return in sentence case, and that warning is a footnote under the Power card. About shows identity, version, Local Only, diagnostics, and the copyright line without promotional taglines.
